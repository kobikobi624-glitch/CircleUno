...
const callAI = async (system, messages) => {
  ...
  return data.content?.[0]?.text || "אני כאן איתך. ספר לי עוד.";
};
...
export default function AISupport({ patientCode, onBack }) {
  ...
  const submitIntakeStep = async (value) => {
    ...
    if (stepIdx < STEPS.length - 1) {
      setStepIdx(i => i + 1);
      setTextVal(""); setSliderVal(50);
    } else {
      setPhase("chat");
      setLoading(true);
      const firstMsg = `סיימתי את השאלות הראשוניות. הטריגר שלי: "${newIntake.trigger}". רמת חרדה: ${newIntake.anxiety}. ${newIntake.urge}.`;
      const initialMessages = [{ role: "user", content: firstMsg }];
      setMessages([{ role: "user", content: firstMsg, hidden: true }]);
      try {
        const reply = await callAI(buildSystemPrompt(newIntake), initialMessages);
        setMessages([{ role: "user", content: firstMsg, hidden: true }, { role: "assistant", content: reply }]);
      } catch {
        setMessages([{ role: "user", content: firstMsg, hidden: true }, { role: "assistant", content: "אני כאן איתך. לא הצלחתי להתחבר — נסה שוב." }]);
      }
      setLoading(false);
    }
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      const reply = await callAI(
        buildSystemPrompt(intake),
        newMessages.filter(m => !m.hidden).map(m => ({ role: m.role, content: m.content }))
      );
      ...
      <div style={{ minHeight: "100vh", background: "#f1f5f9", direction: "rtl", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ maxWidth: 480, width: "100%" }}>
            ...
            <div className="card">
              <p style={{ fontSize: 16, fontWeight: 600, marginBottom: 20, lineHeight: 1.6 }}>{currentStep.question}</p>

              {currentStep.type === "text" && (
                <>
                  <textarea rows={3} value={textVal} onChange={e => setTextVal(e.target.value)}
                    placeholder="כתוב כאן..." style={{ resize: "none", marginBottom: 12 }} autoFocus />
              ...
              {currentStep.type === "slider" && (
                <>
                  <div style={{ textAlign: "center", marginBottom: 12 }}>
                    <span style={{ fontSize: 48, fontWeight: 800, color: sliderVal >= 70 ? "#ef4444" : sliderVal >= 40 ? "#f59e0b" : "#22c55e" }}>{sliderVal}</span>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f1f5f9", direction: "rtl", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ background: "#eef2ff", padding: "8px 16px", fontSize: 12, color: "#4f46e5", borderBottom: "1px solid #e2e8f0" }}>
      ...
      <div style={{ padding: "12px 16px", background: "white", borderTop: "1px solid #e2e8f0", display: "flex", gap: 8 }}>
        <input value={input} onChange={e => setInput(e.target.value)}
          ...
          placeholder="כתוב הודעה..." style={{ flex: 1 }} disabled={loading} />
        <button onClick={sendMessage} disabled={loading || !input.trim()} style={{
          padding: "10px 16px", borderRadius: 10, border: "none", fontWeight: 600, fontSize: 14,
