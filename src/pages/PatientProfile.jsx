...
export default function PatientProfile({ patient, onBack }) {
  ...
  const sorted = [...patient.entries].sort((a,b)=>new Date(a.timestamp)-new Date(b.timestamp));
  ...
  return (
    <div style={{ padding: "16px 20px", maxWidth: 900, margin: "0 auto", direction: "rtl" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
        <button onClick={onBack} style={backBtn}>⬅ חזרה</button>
        <div>
        ...
        <span
          style={{
            marginRight: "auto", padding: "4px 12px", borderRadius: 999,
            fontSize: 12, fontWeight: 700,
            background: riskBadge.bg, color: riskBadge.color,
          }}
        >
          {riskBadge.label}
        </span>
          ...
          <div style={{ width: "100%", height: 220 }}>
            <ResponsiveContainer>
              <LineChart data={chartData} margin={{ top: 4, right: 10, left: -20, bottom: 0 }}>
                ...
                <ReferenceLine y={40} stroke="#f59e0b" strokeDasharray="4 4" label={{ value: "40", fill: "#f59e0b", fontSize: 11 }} />
                <Line
                  type="monotone" dataKey="anxiety" stroke="#6366f1"
      ...
      <div className="card" style={{ marginBottom: 14 }}>
        ...
        <p style={{ fontSize: 13, color: "#64748b", margin: "0 0 10px" }}>
          מסכם אירועים, טריגרים ומגמות של 7 הימים האחרונים
        </p>
        <button
          onClick={() => generateWeeklyReport(patient)}
          style={{
          ...
          {[...sorted].reverse().map(e => (
            ...
            >
              ...
              <div style={{ display: "flex", gap: 12, fontSize: 13, flexWrap: "wrap" }}>
                ...
                <span>
                  <b>חרדה:</b>{" "}
                  <span style={{ fontWeight: 700, color: e.anxiety>=70?"#ef4444":e.anxiety>=40?"#f59e0b":"#22c55e" }}>
                    {e.anxiety}
                  </span>
                </span>
                <span>
                  <b>תגובה:</b>{" "}
                  {e.response==="no"?"נמנע":e.response==="partial"?"חלקי":"התמודד"}
                </span>
              </div>
            </div>
          ))}
...
const backBtn = {
  ...
  background: "white", cursor: "pointer", fontSize: 13, whiteSpace: "nowrap",
};
